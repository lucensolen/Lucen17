(() => {
  ...
  dialRC.addEventListener("input", () =>
    localStorage.setItem(rcKey, dialRC.value)
  );
  dialGE.addEventListener("input", () =>
    localStorage.setItem(geKey, dialGE.value)
  );
  ...
  function apiBase() {
    return (
      localStorage.getItem("lucen.api") ||
      "https://lucen17-backend.onrender.com"
    );
  }
  ...
  function classifyTone(text) {
    ...
    if (/(do|plan|next|build|deploy|fix)/.test(t)) return "Directive";
    if (/(idea|create|vision|dream|invent)/.test(t)) return "Creative";
    return "Reflective";
  }
  ...
  async function logReflection() {
    ...
    try {
      ...
      else console.warn("⚠️ Backend save failed, keeping local copy.");
    } catch (err) {
  ...
  function driftFromTone(tone) {
    ...
    let nrc = rc,
      nge = ge;
    if (tone === "Creative") {
      nrc = Math.min(100, rc + 3);
      nge = Math.min(100, ge + 2);
    } else if (tone === "Directive") {
      nrc = Math.max(0, rc - 2);
      nge = Math.max(0, ge - 1);
    } else {
      nrc = Math.max(0, Math.min(100, rc + (Math.random() * 2 - 1)));
    }
    dialRC.value = String(nrc);
    dialGE.value = String(nge);
  ...
  let lucenDivisions =
    JSON.parse(localStorage.getItem("lucen.divisions")) || defaultDivisions;
  ...
  // === Beam Tone System (smooth blended update) ===
  function updateBeamTone() {
    if (!beam) return;
    const moods = Array.from(
      document.querySelectorAll('[data-field="mood"]')
    )
  ...
  payBtn?.addEventListener("click", async () => {
    ...
    try {
      const r = await postJSON(`${base}/tolls/pay`, {
        gate,
        amount: amt,
        currency: "GBP"
      });
      if (r.simulated) alert("Simulated payment ok");
      else if (r.client_secret)
        alert("PaymentIntent created (test only)");
